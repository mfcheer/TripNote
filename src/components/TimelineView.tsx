...
function TripStatsBar({ trip, onOpenBudget }: { trip: Trip; onOpenBudget: () => void }) {
  ...
  const stats = [
    { label: '天数', value: `${trip.days.length} 天`, mobileValue: `${trip.days.length} 天`, Icon: CalendarIcon },
    { label: '安排', value: `${trip.activities.length} 项`, mobileValue: `${trip.activities.length} 项`, Icon: OverviewIcon },
    { label: '已定位', value: `${trip.activities.filter((activity) => activity.geo).length} 个`, mobileValue: `${trip.activities.filter((activity) => activity.geo).length} 个`, Icon: PinIcon },
    {
      ...
      mobileValue: `¥${totalCost.toLocaleString()}`,
      ...
      {stats.map(({ label, value, mobileValue, hint, Icon, onClick }, index) => {
        ...
        const content = <>
          ...
          <span className="flex min-w-0 flex-col gap-0 sm:flex-row sm:items-baseline sm:gap-1.5">
            <span className="shrink-0 text-[10.5px] text-text-muted sm:text-[11.5px]">{label}</span>
            <span className="truncate text-[12.5px] font-semibold text-text sm:hidden">{mobileValue}</span>
            <span className="hidden truncate text-[13.5px] font-semibold text-text sm:inline">{value}</span>
...
function DayOverview({ items, scheduleWarningCount, compact = false, onOpenBudget }: { items: Activity[]; scheduleWarningCount: number; compact?: boolean; onOpenBudget?: () => void }) {
  ...
  return (
    <div className={`${compact ? 'mb-2 gap-x-2 text-[11px]' : 'mb-4 gap-x-3 text-[12px]'} flex flex-wrap items-center gap-y-1 px-0.5 text-text-muted`}>
      ...
      {onOpenBudget && <button onClick={onOpenBudget} className="inline-flex items-center gap-1 rounded px-1 py-0.5 text-[10.5px] font-medium text-text-muted transition-colors hover:bg-surface hover:text-accent" title="查看旅行总预算"><WalletIcon size={12} /> 预算</button>}
...
function AddActivityForm({ dayId, onDone, compact = false }: { dayId: string; onDone: () => void; compact?: boolean }) {
  ...
  function savePlace(place: GeoResult) {
    ...
    useToastStore.getState().show('已加入想去')
    setResults([])
    setResultIndex(-1)
    setSearchedQuery('')
    setTitle('')
...
function DayHeaderInfo({ day, mobilePresentation = false }: { day: { id: string; date: string; place: string }; mobilePresentation?: boolean }) {
  ...
  if (!editing) {
    ...
    return (
      <button
        ...
        title="点击编辑日期/地点"
      >
        {showDate && (!mobilePresentation || !showPlace) && (
...
function DaySection({
  ...
  const [nativeDropConfirmed, setNativeDropConfirmed] = useState(false)
  ...
  useEffect(() => {
    if (!nativeDropConfirmed) return
    const timer = window.setTimeout(() => setNativeDropConfirmed(false), 620)
  ...
  return (
      <section id={`workspace-day-${dayId}`} ref={setNodeRef} onDragOver={(event) => { if (acceptsWish(event)) { event.preventDefault(); event.dataTransfer.dropEffect = 'copy'; setNativeWishOver(true) } }} onDragLeave={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setNativeWishOver(false) }} onDrop={(event) => { if (!acceptsWish(event)) return; setNativeWishOver(false); setNativeDropConfirmed(true); onNativeWishDrop?.(event, dayId) }} className={`${compact ? 'mb-3 scroll-mt-[48px] pb-3' : 'mb-5 pb-5 sm:mb-7 sm:pb-7'} border-b border-border/70 transition-colors last:border-b-0`}>
        {/* 天标题 */}
...
function PlannerInspector({
  ...
  const [panelWidth, setPanelWidth] = useState(() => readPanelWidth(PLANNER_PANEL_WIDTH_KEY, 400, 360, 680))
  const day = trip.days.find((item) => item.id === dayId)
  ...
  return (
    <>
      <div role="separator" aria-label="调整行程地图与详情宽度" aria-orientation="vertical" onPointerDown={startPanelResize} className="group hidden w-2 shrink-0 cursor-col-resize touch-none items-center justify-center xl:flex">
        <span className="h-11 w-px rounded-full bg-border transition-colors group-hover:bg-accent" />
      </div>
      <aside className="sticky top-0 hidden h-[calc(100vh-52px)] min-w-0 max-w-[45vw] shrink-0 overflow-y-auto border-l border-border/80 bg-[#fafbfb] xl:block" style={{ width: panelWidth }}>
...
function MobileDayStrip({ trip }: { trip: Trip }) {
  ...
  return (
    <div className="-mx-3 mb-3 flex items-stretch gap-2 overflow-x-auto px-3 pb-1 sm:hidden">
      {trip.days.map((day) => {
...
export default function TimelineView({ onOpenFullMap, onOpenBudget, workspace = false, showAllDays = false, onShowAllDays, onFocusDay, hideQuickAdd = false, introducedActivityId = null, mobilePresentation = false, quickAddRequest = 0 }: { onOpenFullMap: () => void; onOpenBudget?: () => void; workspace?: boolean; showAllDays?: boolean; onShowAllDays?: () => void; onFocusDay?: (dayId: string) => void; hideQuickAdd?: boolean; introducedActivityId?: string | null; mobilePresentation?: boolean; quickAddRequest?: number }) {
  ...
  const [recentlyDroppedDayId, setRecentlyDroppedDayId] = useState<string | null>(null)
  ...
  function onDragOver(event: DragOverEvent) {
    const overData = event.over?.data.current as { dayId?: string } | undefined
    setDragOverDayId(overData?.dayId ?? null)
  }
  ...
  useEffect(() => {
    if (!recentlyDroppedDayId) return
    const timer = window.setTimeout(() => setRecentlyDroppedDayId(null), 620)
    return () => window.clearTimeout(timer)
  }, [recentlyDroppedDayId])
  ...
  return (
    <DndContext
      ...
      onDragEnd={onDragEnd}
    >
      <div className={`timeline-root flex min-h-full min-w-0 ${mobilePresentation ? 'mobile-timeline' : ''}`}>
        <div className="min-w-0 flex-1">
          <div className={`${workspace ? 'max-w-none px-5 py-5' : mobilePresentation ? 'mr-auto max-w-[980px] px-3.5 pt-1.5 pb-3 sm:px-7 sm:py-7 lg:px-10' : 'mr-auto max-w-[980px] px-3 py-4 sm:px-7 sm:py-7 lg:px-10'}`}>
            ...
            {!workspace && !mobilePresentation && <div className="mb-3 flex items-center justify-between sm:mb-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-md border border-border/80 bg-white/85 px-2.5 py-1.5 text-[12px] font-medium text-text-muted shadow-[0_1px_2px_rgba(32,40,46,0.04)] transition-colors hover:border-accent/45 hover:text-text active:bg-surface"
              >
