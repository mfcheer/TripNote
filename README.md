...
北向适合在出发前做旅行规划，也适合旅途中打开手机快速查阅。它不需要注册账号；默认情况下，所有行程数据只保存在你当前浏览器中。

## 界面预览

以下截图均来自内置的「东北大环线（示例）」行程。
...
- **多旅行管理**：新建、切换和管理不同旅行；内置一份可随时删除的东北大环线示例行程。
...
- **可选地图服务**：默认使用 OpenStreetMap / Nominatim；在设置中填入自己的高德 Key 后，可切换到高德地图和地点搜索。也可填入 MapTiler Key 启用“中文标注地图”，让日韩等海外地区优先显示中文地名。密钥不会写入代码或上传到服务器。
...
2. 在北向的“设置 → 高德地图（可选）”中填入 JS API Key 与 Web 服务 Key。
